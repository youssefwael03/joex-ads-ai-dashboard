...
function exportCsv(rows: any[]) {
  ...
  const lines = rows.map((ad: any) => {
    const d = ad.insights?.data?.[0] ?? {};
    return [
      `"${ad.name}"`,
      ad.status,
...
export default function Ads() {
  ...
  const ads = data?.data ?? [];
  const filtered = ads.filter((a: any) => a.name?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Ad creative performance metrics.</p>
        </div>
                  ...
                  >
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium line-clamp-1 max-w-[180px]">{ad.name}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge
